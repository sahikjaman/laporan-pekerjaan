...
import {
  ...
  taskToFormData,
  taskToDisplay,
  progressLogFormToData,
  progressLogToDisplay,
  sparepartFormDataToSparepart,
  sparepartToFormData,
  sparepartToDisplay,
  repairFormDataToRepair,
  repairToFormData,
  repairToDisplay,
} from "./dataMappers";
...
export default function LaporanPekerjaan() {
  ...
  const loadSpareparts = async () => {
    try {
      ...
      const displayData = data.map(sparepartToDisplay);
      setSpareparts(displayData);
    } catch (error) {
  ...
  const loadRepairs = async () => {
    try {
      ...
      const displayData = data.map(repairToDisplay);
      setRepairs(displayData);
    } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const reportData = formDataToReport(formData);

      if (editingId) {
        await reportsAPI.update(editingId, reportData);
        await loadReports();
        setEditingId(null);
      } else {
  ...
  const handleTaskSubmit = async () => {
    ...
    try {
      ...
      if (editingTaskId) {
        await tasksAPI.update(editingTaskId, taskData);
        await loadTasks();
  ...
  const handleSparepartSubmit = async () => {
    ...
    setSaving(true);
    try {
      const sparepartData = sparepartFormDataToSparepart(sparepartFormData);

      if (editingSparepartId) {
        await sparepartsAPI.update(editingSparepartId, sparepartData);
      } else {
  ...
  const handleUpdateSparepartDates = async () => {
    ...
    try {
      ...
      await sparepartsAPI.update(selectedSparepart.id, updates);
      await loadSpareparts();
      setShowSparepartDateModal(false);
  ...
  const handleUpdateSparepartStatus = async (sparepart, newStatus) => {
    ...
    try {
      ...
      await sparepartsAPI.update(sparepart.id, updates);
      await loadSpareparts();
    } catch (error) {
